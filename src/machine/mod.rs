...
use crate::plate::*;
use crate::SupportedTypes::{Float, Int};
...
impl VM {
    ...
    fn validate(&mut self, program: Vec<u8>) {
        ...
        // Change all magic numbers to the following four digits.
        self.headers.magic0 = rv[0];
        self.headers.magic1 = rv[1];
        self.headers.magic2 = rv[2];
        self.headers.magic3 = rv[3];
        ...
        self.headers.version = rv[4] as usize;
    ...
    fn decode(&mut self, program: u8) -> Bytecodes {
        ...
        return match program {
            ...
            39 => { self.string.push('\''); NULL }
            40 => { self.string.push('('); NULL }
            41 => { self.string.push(')'); NULL }
            44 => { self.string.push(','); NULL }
            45 => { self.string.push('-'); NULL }
            46 => { self.string.push('.'); NULL }
            47 => { self.string.push('/'); NULL }
            58 => { self.string.push(':'); NULL }
    ...
    // Finally, execute it.
    // TODO: Merge ILOAD, FLOAD into one function. 
    fn execute(&mut self, program: Bytecodes) {
        ...
        match program {
            ...
            TAKE => {   let DUMMY = self.plate.take();    }
            ...
            MERGE => {
                let apple = self.plate.take();
                let banana = self.plate.take();
                let seasoning = GetValueOfType {
                    int: apple.value.int.clone(),
                    float: apple.value.float.clone(),
            ...
            ADD => {
                ...
                let result = apple.value.int + banana.value.int;
                let mut seasoning = GetValueOfType {
                    int: result,
                    float: 0.0,
                    str: String::from(""),
                };
            ...
            WRITE => {
                syscalls::write(self.plate.peek().value.int, self.plate.peek().value.str.clone());
                self.string = String::from("");
            }
